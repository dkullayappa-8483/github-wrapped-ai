...
// Initialize Gemini with a check
...
async function fetchGitHubData(username) {
  ...
  try {
    const userRes = await fetch(`https://api.github.com/users/${username}`, { headers, timeout: 10000 });
    ...
    const reposRes = await fetch(`https://api.github.com/users/${username}/repos?per_page=100&sort=pushed`, { headers, timeout: 10000 });
    ...
    return {
      ...
      recent_repos: repos.slice(0, 8).map(r => ({ name: r.name, description: r.description, stars: r.stargazers_count }))
    };
...
// Global Error Handler to catch anything outside routes
